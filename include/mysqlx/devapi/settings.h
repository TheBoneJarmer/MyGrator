...
/**
  ...
  connect options (db_host, port, user, password, database) or with a list
  of `SessionOption` constants, each followed by the option value.

  Examples:
  ~~~~~~

    SessionSettings from_url("mysqlx://user:pwd@host:port/db_scheme?ssl-mode=required");

    SessionSettings from_options("db_host", port, "user", "pwd", "db_scheme");

    SessionSettings from_option_list(
      SessionOption::USER, "user",
      SessionOption::PWD,  "pwd",
      SessionOption::HOST, "db_host",
      SessionOption::PORT, port,
      SessionOption::DB,   "db_scheme",
  ...
  description of `Session` class). In that case each db_host can be assigned
...
public:
  ...
  /**
    ...
    Connection sting has the form
          ...
          "user:pass@connection-data/db_scheme?option&option"

    with optional `mysqlx://` prefix.

    The `connetction-data` part is either a single db_host address or a coma
    ...
    A single db_host address is either a DNS db_host name, an IPv4 address of
    the form `nn.nn.nn.nn` or an IPv6 address of the form `[nn:nn:nn:...]`.
    On Unix systems a db_host can be specified as a path to a Unix domain
    ...
    encoding (e.g., `%2F` for `/`). Another option is to enclose a db_host name or
    a socket path in round braces. For example, one can write

        "mysqlx://(./path/to/socket)/db_scheme"

    instead of

        "mysqlx://.%2Fpath%2Fto%2Fsocket/db_scheme"

    To specify priorities for hosts in a multi-db_host settings, use list of pairs
    of the form `(address=db_host,priority=N)`. If priorities are specified, they
    must be given to all hosts in the list.

    The optional `db_scheme` part of the connection string defines the default schema
    of the session.
  ...
  */
  ...
  /**
    ...
    db_host, all have to be specified in the same `set()` call.
   */
